...
WELCOME_PHOTO_FILE_ID = os.environ.get('WELCOME_PHOTO_FILE_ID', '')  # Optional: Telegram file_id
...
def handle_start(message):
    """Handle /start command - creates user & sends dashboard link with welcome photo"""
    try:
        ...
        # Welcome message with photo caption
        welcome_caption = f"""
...
🔗 Tap below to open your dashboard and start earning!
        ...
        # Send welcome with photo (3 methods - choose best available)
        sent = False
        
        # Method 1: Telegram file_id (most reliable)
        if WELCOME_PHOTO_FILE_ID and not sent:
            try:
                bot.send_photo(
                    message.chat.id, 
                ...
                logger.warning(f"file_id method failed: {e}")
        
        # Method 2: Public URL (Vercel hosted)
        if WELCOME_PHOTO_URL and not sent:
            try:
                bot.send_photo(
                    message.chat.id, 
        ...
        # Method 3: Local file (for testing)
        if not sent:
            try:
                local_path = os.path.join(os.path.dirname(__file__), '../static/welcome.jpg')
                ...
                logger.warning(f"Local file method failed: {e}")
...
# Serve static files (for welcome photo)
