    ...
    if request.method == "POST":
        try:
            ...
            if update.message:
                ...
                if init_fb():
                    db.reference(f'users/{chat_id Burg}').update({"name": u_name})
...
def dashboard():
    ...
    init_fb()
    u_data = db.reference(f'users/{uid}').get() or {"pts": 0, "coupon": "NEW-USER"}
    ...
    <style>
        ...
        .task { background:#1e293b; padding:15px; border-radius:12px; margin-top:10px; display:flex; justify-content:space-between; border:1px solid #334155; text-decoration:none; color:white;}
    </style></head><body>
        <div class="card"><p>Points Balance</p><div class="pts">{{pts}}</div></div>
        <div style="text-align:left; margin-top:20px;">
            <a href="/dashboard?id={{uid}}&name={{name}}&ad_claim=1" class="task"><div>Watch Ad</div><b>+10</b></a>
        </div>
    </body></html>
    """, pts=u_data.get('pts', 0), uid=uid, name=name)
